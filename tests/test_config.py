...
def options(monkeypatch):
    ...
    monkeypatch.setattr(config.papis.config, "getstring", lambda *args: "about 200 words")
...
def test_chunk_override_keeps_upstream_model_controls(options, monkeypatch):
    ...
    monkeypatch.setattr(config, "_get_optional_string", lambda key: "custom-vision" if key == "enrichment-llm" else None)
